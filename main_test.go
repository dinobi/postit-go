...
func TestApiHandler(t *testing.T) {
	...
	// Create an HTTP handler from our handler function. "apiHandler" is the handler
	// function defined in our main.go file that we want to test
	hf := http.HandlerFunc(apiHandler)

	// Serve the HTTP request to our recorder. This is the line that actually
	// executes the apiHandler that we want to test
	hf.ServeHTTP(recorder, req)

	// Here is our actual assertions
...
func TestRouter(t *testing.T) {
	...
	// 2) In the next few lines, the response body is read, and converted to a string
	defer resp.Body.Close() // close the response body when client has finished transaction
...
func TestStaticFileServer(t *testing.T) {
	...
	if expectedContentType != "text/html; charset=utf-8" {
